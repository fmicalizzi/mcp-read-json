...
export class KnowledgeBaseManager {
  ...
  async loadKnowledgeBase(): Promise<KnowledgeBase> {
    ...
    try {
      ...
      this.knowledgeBase = JSON.parse(data) as KnowledgeBase;
  ...
  async getEntriesByCategory(categoryId: string): Promise<Entry[]> {
    const knowledgeBase = await this.loadKnowledgeBase();
  ...
  async getEntriesByTags(tags: string[]): Promise<Entry[]> {
    const knowledgeBase = await this.loadKnowledgeBase();
  ...
  async getRelatedEntries(entryId: string): Promise<Entry[]> {
    ...
    return knowledgeBase.entries.filter(e => 
      entry.related.includes(e.id)
    );
  }
  ...
  async getKnowledgeStats(): Promise<KnowledgeBaseStats> {
    ...
    knowledgeBase.categories.forEach(cat => {
      entriesByCategory[cat.id] = knowledgeBase.entries.filter(
    ...
    const totalRelated = knowledgeBase.entries.reduce(
      (sum, entry) => sum + entry.related.length, 
      0
    );
